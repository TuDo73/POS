...
export default {
  ...
  async setRefOrder(ref, userCode) {
    ...
    let setRefOrderResponse = await globalFunction.request(
      urlSetRefOrder,
      orderRefObj
    );
  },
  ...
  async getRefOrder(userCode) {
    ...
    let getRefOrderResponse = await globalFunction.request(
      urlGetRefOrder,
      orderRefObj
    );
  ...
  async addOrder(order) {
    ...
    for (let i = 0; i < orderTmp.orderLines.length; i++) {
      if (
        orderTmp.orderLines[i].product_id == "" ||
        !globalFunction.checkValid(orderTmp.orderLines[i].product_id)
      ) {
        orderTmp.orderLines[i].product_id =
          i + orderTmp.orderLines[i].code + orderTmp.ref;
      }
    }
