...
const actions = { // methods
  initOrder({commit, state, dispatch, rootState}, refNumber) {
    ...
    if(table.isTogo) { // trường hợp là order to-go
      ...
      if(refOrderTmp !== '') { // trường hợp chọn 1 order to-go đã có
      ...
      } else { // trường hợp tạo mới 1 order to-go
        for(let i = 0; i < state.orders.length; i ++) {
          state.orders[i].isActive = false;
        }
      ...
      if(state.orders.length === 0) { // nếu chưa tồn tại order nào
        dispatch('createOrder');
      } else { // đã tồn tại order
        ...
        let orderExit = state.orders.some(order => order.tableCode == table.code); // kiểm tra xem đã có order thuộc bàn đã chọn hay chưa
        if (!orderExit) { // nếu bàn đã chọn chưa có order nào
          for (let i = 0; i < state.orders.length; i ++) {
            if (state.orders[i].isActive) {
              state.orders[i].isActive = false;
            }
          ...
          dispatch('createOrder');
        } else { // nếu bàn đã chọn đã có order
  ...
  setProductToOrderLines({commit, state, dispatch, rootState}, [product]) {
    ...
    for(let i = 0; i < orderLines.length; i++) {
      ...
      if(orderLines[i].code === tmpProduct.code && orderLines[i].isDisableSelect === false && orderLines[i].isOrderConfirmed === false) {
        if(rootState.numboardHome.isMenge) {
          orderLines[i].quantity = tmpProduct.quantity;
        } else {
          orderLines[i].quantity = parseFloat(orderLines[i].quantity) + parseFloat(tmpProduct.quantity);
        }
  ...
  setOrderLine({commit, state, dispatch, rootState}, [orderLine, indexOrderLine]) {
    ...
    for(let i = 0; i < orderLinesTmp.length; i++) {
      ...
      if(orderLinesTmp[i].code === orderLine.code && orderLinesTmp[i].isDisableSelect === false && indexOrderLine === i) {
  ...
  doStorno({commit, state, dispatch, rootState}) {
    ...
    function findNextSelectableRowIndex() {
      for(let i = state.currentSelectedRowIndex + 1; i < orderLines.length; i++) {
    ...
    if(state.shouldDelete) {
      if(orderLines[state.currentSelectedRowIndex].isHighlight && !orderLines[state.currentSelectedRowIndex].isOrderConfirmed) { // sản phẩm ở trạng thái chưa confirm
      ...
      } else if(orderLines[state.currentSelectedRowIndex].isHighlight && orderLines[state.currentSelectedRowIndex].isOrderConfirmed) { // sản phẩm ở trạng thái đã confirm
        ...
        orderLines[state.currentSelectedRowIndex].reasonStorno = state.reasonStorno;
        ...
        if(nextRowIndex != -1) {
          state.currentSelectedRowIndex = smProductIndex < nextRowIndex ? nextRowIndex + 1 : nextRowIndex;
          orderLines[state.currentSelectedRowIndex].isHighlight = true;
        } else {
          state.shouldDelete = false;
        }
  ...
  doStornoAll({commit, state, dispatch, rootState}, isOrderConfirmExiting) {
    ...
    if (isOrderConfirmExiting) { // order list tồn tại món đã confirm
      for (let i = orderLines.length; i--;) {
        if (orderLines[i].isOrderConfirmed && orderLines[i].isDisableSelect == false) { // món đã confirm
          orderLines[i].isDisableSelect = true;
          orderLines[i].isHighlight = false;
  
          stornoItems.push(orderLines[i]);
        } else if (!orderLines[i].isOrderConfirmed) { // món chưa confirm
      ...
      for (let i = orderLines.length; i--;) {
        if (!orderLines[i].isOrderConfirmed && orderLines[i].isDisableSelect == false) {
          orderLines.splice(i, 1);
        }
      }
    }
      ...
      case 'up':
        if(currentRowIndex <= firstRowIsActive) {
          ...
          for(let i = 0; i < orderLines.length; i++) {
            ...
            if(orderLines[currentRowIndex].isDisableSelect == false && currentRowIndex >= firstRowIsActive) {
              globalFunction.scrollToElement('order-list-body', 'row-ol', currentRowIndex);
              break;
            }
          }
        }
        ...
        if(currentRowIndex >= lastRowIsActive) {
          ...
          for(let i = 0; i < lastRowIsActive; i++) {
            ...
            if(currentRowIndex >= lastRowIsActive) {
              ...
              if(orderLines[currentRowIndex].isDisableSelect == false && currentRowIndex <= lastRowIsActive) {
                globalFunction.scrollToElement('order-list-body', 'row-ol', currentRowIndex);
                break;
              }
            }
  ...
  calculateOrder({commit, state, dispatch, rootState}) {
    ...
    for (let i = 0; i < orderLinesTmp.length; i++) {
      if (orderLinesTmp[i].discountType === '%') {
        if(orderLinesTmp[i].quantity < 0) {
          totalAmount = ((Number(orderLinesTmp[i].price) + (Number(orderLinesTmp[i].discount) / 10)) * Number(orderLinesTmp[i].quantity)) * -1;
          if (Number(totalAmount) > 0) {
            totalAmount = Number(totalAmount) * -1;
          }
        } else {
          totalAmount = (Number(orderLinesTmp[i].price) - (Number(orderLinesTmp[i].discount) / 10)) * Number(orderLinesTmp[i].quantity);
        }
        ...
        if(orderLinesTmp[i].priceAfterChange != 0) {
          totalAmount = Number(orderLinesTmp[i].priceAfterChange) * Number(orderLinesTmp[i].quantity);
          if(orderLinesTmp[i].priceAfterChange <= orderLinesTmp[i].priceOrigin) {
            orderLinesTmp[i].discount = Number(orderLinesTmp[i].priceOrigin) * Number(orderLinesTmp[i].quantity) - Number(totalAmount);
          } else {
            orderLinesTmp[i].discount = 0;
          }
        } else {
          if(orderLinesTmp[i].quantity < 0) {
            totalAmount = (Number(orderLinesTmp[i].price) * Number(orderLinesTmp[i].quantity) + Number(orderLinesTmp[i].discount)) * -1;
            if (Number(totalAmount) > 0) {
              totalAmount = Number(totalAmount) * -1;
            }
          } else {
            totalAmount = Number(orderLinesTmp[i].price) * Number(orderLinesTmp[i].quantity) - Number(orderLinesTmp[i].discount);
          }
        }
        orderLinesTmp[i].totalAmount = Number(totalAmount).toFixed(2);
      ...
      if (orderLinesTmp[i].priceAfterChange > 0) {
        totalPriceWithoutDiscount += Number(orderLinesTmp[i].quantity) * Number(orderLinesTmp[i].priceAfterChange);
      } else {
        totalPriceWithoutDiscount += Number(orderLinesTmp[i].quantity) * Number(orderLinesTmp[i].priceOrigin);
      }
    ...
    if (orderSelectedTmp.orderDiscountValue != 0) {
      ...
      orderSelectedTmp.moneyToPay = Number(orderSelectedTmp.moneyToPay) - Number(orderSelectedTmp.orderDiscountValue);
    }
    orderSelectedTmp.moneyGiven = orderSelectedTmp.moneyToPay;
    orderSelectedTmp.moneyChange = 0;
    orderSelectedTmp.totalPriceWithoutDiscount = Number(totalPriceWithoutDiscount).toFixed(2);
    orderSelectedTmp.totalDiscountOrderLines = Number(totalDiscountOrderLines).toFixed(2);
    ...
    for (let orderTmp of ordersTmp) {
      ...
      if (orderTmp.ref === orderSelectedTmp.ref) {
        for(let orderline of orderLinesTmp) {
          ...
          if(Number(orderline.priceAfterChange) > 0) {
            totalPriceWithoutDiscount += Number(orderline.quantity) * Number(orderline.priceAfterChange);
          } else {
            totalPriceWithoutDiscount += Number(orderline.quantity) * Number(orderline.priceOrigin);
          }
        }
      } else {
      ...
      orderTmp.totalPriceWithoutDiscount = Number(totalPriceWithoutDiscount).toFixed(2);
    }
  ...
  handleTaxOrder({commit, state, dispatch, rootState}) {
    ...
    for (let orderLine of orderSelectedTmp.orderLines) {
      if (orderLine.discountType == '%') { // chuyển giá trị discount của món về Euro
        orderLine.discountType = '€';
        
        let discountValue = orderLine.discount;
        orderLine.discount = Number(orderLine.priceOrigin) * Number(orderLine.quantity) * (Number(discountValue) / 100);
      }
    }

    if (orderSelectedTmp.orderDiscountValue !== 0) {
      if (orderSelectedTmp.orderDiscountType == '%') { // chuyển giá trị discount của order về Euro
        orderSelectedTmp.orderDiscountValue = (Number(orderSelectedTmp.totalPrice) * Number(orderSelectedTmp.orderDiscountValue) / 100);
    ...
    // console.log('totalPrice7:', totalPrice7);
    // console.log('totalPrice19:', totalPrice19);
    ...
    // console.log('totalMwst7:', totalMwst7);
    // console.log('totalMwst19:', totalMwst19);
    ...
    // console.log('rateTotalPrice7:', rateTotalPrice7);
    // console.log('rateTotalPrice19:', rateTotalPrice19);
    ...
    totalPrice7 = Number(totalPrice7) - (Number(rateTotalPrice7) * Number(orderDiscountValue) / 100);
    totalPrice19 = Number(totalPrice19) - (Number(rateTotalPrice19) * Number(orderDiscountValue) / 100);
    ...
    // console.log('nettoTotalPrice7:', nettoTotalPrice7);
    // console.log('nettoTotalPrice19:', nettoTotalPrice19);
    ...
    totalDiscountOrder = Number(totalDiscountOrderLines) + Number(orderSelectedTmp.orderDiscountValue);
  ...
  doMoveDish({commit, state, dispatch, rootState}) {
    ...
    for (let i = 0; i < state.orderTargetMoveDish.orderLines.length; i++) {
      totalAmount += Number(state.orderTargetMoveDish.orderLines[i].totalAmount);
    }
    state.orderTargetMoveDish.totalPrice = totalAmount;
  },

  handleCurrentOrderAfterMoveDish({commit, state, dispatch, rootState}, currentProducts) {
    let tmpObj = {};
    ...
    state.orderSelected.orderLines =
      state.orderSelected.orderLines.filter(order => tmpObj[order.codeToMove] || order.isDisableSelect);

    dispatch('calculateOrder');
  },

  handleOrderReceiveMoveDish({commit, state, dispatch, rootState}, orderReceive) {
    for (let order of state.orders) {
...
const mutations = { // handle response from actions to update state
    ...
    if(globalFunction.checkValid(state.orderSelected.orderLines)) {
      for(let i = 0; i < state.orderSelected.orderLines.length; i++) {
        if(state.orderSelected.orderLines[i].isHighlight) {
          ...
          // console.log('state.currentSelectedRowIndex 1:', state.currentSelectedRowIndex);
          return;
        }
      }
    }

    // console.log('state.currentSelectedRowIndex 2:', state.currentSelectedRowIndex);
    state.shouldDelete = false;
    return;
  },
  ...
  setOrderToMoveDish(state) {
    for (let i = 0; i < state.orderSelected.orderLines.length; i++) {
      state.orderSelected.orderLines[i].codeToMove = i + state.orderSelected.orderLines[i].code; // thêm field codeToMove của orderline để xử lý nhiều món trùng nhau trên list
    }
    
    state.orderToMoveDish = globalFunction.deepCloneObj(state.orderSelected);
    state.orderToMoveDish.orderLines = state.orderToMoveDish.orderLines.filter(order => !order.isDisableSelect);
