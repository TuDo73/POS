...
<script>
  ...
  export default {
    ...
    sockets: {
      ...
      ALREADY_LOGIN(data) {
        if(this.user.username == data.userName && this.user.loginTime == data.loginTime) {
      ...
      CONFIRM_MOVE_DISH(data) {
        if (globalFunction.checkValid(this.user.username)) {
          ...
          let currentOrderLines = globalFunction.deepCloneObj(this.orderToMoveDish.orderLines);

          if (data.isNewTable) { // khi bàn rỗng
            let userNameTarget = data.userCodeTarget;
  
            if (this.user.username == userNameTarget) { // chỉ thực hiện move dish ở bàn thuộc bồi muốn move dish
              // xử lý data của bàn hiện tại
              this.$store.dispatch('order/handleCurrentOrderAfterMoveDish', currentOrderLines);

              this.$store.commit('helper/showLoading', true);
  
              // tạo bàn mới
              let fullName = this.users.filter(user => user.username == userNameTarget)[0].fullname;
              let tableSelected = this.allTablesData.filter(table => table.code == data.tableCode)[0];
  
              this.$store.dispatch('table/selectTable', [tableSelected, userNameTarget, fullName, false]);
              ...
              OrderService.save().then(() => {
                ...
                (async () => {
                  ...
                  let tableResponse = await globalFunction.request(urlTable, tableSelected);
                  ...
                  tableData.userFullName = this.users.filter(user => user.username == newOrder.userCode).reduce((users, user) => {return user.fullname}, []);
            ...
            if (!data.isDifferentUser) { // khi bàn move dish là của bồi hiện tại
              if (this.user.username == data.userCodeTarget) { // chỉ thực hiện move dish ở bàn thuộc bồi muốn move dish
                this.$store.commit('helper/showLoading', true);

                // xử lý data của bàn hiện tại
                this.$store.dispatch('order/handleCurrentOrderAfterMoveDish', currentOrderLines);
    
                this.$store.commit('order/updateOrders', this.orderTargetMoveDish);
                OrderService.save().then(() => {
                  this.$store.dispatch('order/calculateOrder');
                  this.$store.dispatch('popup/updatePopupState', false);
                  this.$store.dispatch('order/showHidePopupMoveDish', false);
            ...
            } else { // khi bàn move dish là của bồi khác
              let userFulNameOrigin = this.users.filter(user => user.username == data.userCodeOrigin)[0].fullname;
              this.$store.commit('tableFunction/setUserMoveDishTarget', userFulNameOrigin);
  
              let tableTarget = this.allTablesData.filter(table => table.code == data.tableCode)[0].name;
              this.$store.commit('tableFunction/setTableMoveDishTarget', tableTarget);
  
              if (this.user.username == data.userCodeOrigin) { // xử lý ở phía bồi muốn move dish
                this.$store.commit('helper/setStateLoadingPopupMoveDish', true);
              }
  
              if (this.user.username == data.userCodeTarget) { // xử lý ở phía bồi nhận move dish
                let currentRoute = this.$route.name;
                if (currentRoute !== 'Payment' && currentRoute !== 'Print') {
                  this.$store.dispatch('popup/updatePopupState', true);
                  this.$store.dispatch('order/showHidePopupConfirmMoveDish', true);
                }
              }
            }
          }
      ...
      RECEIVE_MOVE_DISH() {
        if (globalFunction.checkValid(this.user.username)) {
          if (this.user.username == this.dataMoveDishSocket.userCodeOrigin) { // xử lý ở phía bồi muốn move dish
            let currentOrderLines = globalFunction.deepCloneObj(this.orderToMoveDish.orderLines);
          ...
          if (this.user.username == this.dataMoveDishSocket.userCodeTarget) { // xử lý ở phía bồi nhận move dish
            let currentOrderLines = globalFunction.deepCloneObj(this.dataMoveDishSocket.dataOrder.orderLines);

            // xử lý data của bàn hiện tại
            this.$store.dispatch('order/handleOrderReceiveMoveDish', this.dataMoveDishSocket.dataOrder);
      ...
      CONFIRM_CHANGE_USER(data) {
        if (globalFunction.checkValid(this.user.username)) {
          ...
          if (this.user.username == data.userOrigin) { // xử lý phía user muốn đổi bàn
            this.$store.commit('helper/setStateLoadingPopupChangeUser', true);
          }
  
          if (this.user.username == data.userTarget) { // xử lý phía user nhận bàn
            let currentRoute = this.$route.name;
            if (currentRoute !== 'Payment' && currentRoute !== 'Print') {
              this.$store.dispatch('popup/updatePopupState', true);
              ...
              for (let user of this.users) {
                if (user.username == data.userOrigin) {
                  this.$store.commit('tableFunction/setUserChangeTableOrigin', user);
                }
                if (user.username == data.userTarget) {
                  this.$store.commit('tableFunction/setUserChangeTableTarget', user);
                }
              }
            }
          }
      ...
      CANCEL_CHANGE_USER() {
        if (globalFunction.checkValid(this.user.username)) {
          if (this.user.username == this.dataChangeUserSocket.userOrigin) { // xử lý phía user muốn đổi bàn
            this.$store.dispatch('popup/updatePopupState', false);
            this.$store.dispatch('order/showHidePopupChangeUser', false);
          }

          if (this.user.username == this.dataChangeUserSocket.userTarget) { // xử lý phía user nhận bàn
            this.$store.dispatch('popup/updatePopupState', false);
      ...
      RECEIVE_CHANGE_USER() {
        if (globalFunction.checkValid(this.user.username)) {
          if (this.user.username == this.dataChangeUserSocket.userOrigin) { // xử lý phía user muốn đổi bàn
            ...
            for (let i = 0; i < this.dataChangeUserSocket.tableToMove.length; i++) {
              tmpTablesObj[this.dataChangeUserSocket.tableToMove[i]] = 1;
            }
            
            for (let i = ordersTmp.length; i--;) { // xóa những order ở các bàn đã move
            ...
            this.$store.commit('tableFunction/setOrdersToChangeUser', ordersToChangeUser);
            ...
            OrderService.save().then(() => {
              (async () => {
                ...
                for (let i = 0; i < data.tablesMoving.length; i++) {
                  this.$store.commit('helper/showLoading', true);
                  let tableTmp = globalFunction.deepCloneObj(data.tablesMoving[i]);
                  ...
                  dataTableUser.userFullName = this.users.filter(user => user.username == this.user.username)[0].fullname;
