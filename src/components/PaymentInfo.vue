    ...
    <div class="payment-info" v-if="!isOnSmallScreen">
      <div class="payment-info-top">
        ...
        <div class="before-discount">
          ...
          <span class="before-discount-num">{{
            orderForShow.totalPrice | centToEuroFormat
          }}</span>
        </div>
      </div>
      <div class="payment-info-bottom">
        <div class="products-payment-box included-box">
          ...
          <ul class="products-payment-list" ref="includedList">
            ...
            >
              ...
              <span class="list-item-num"> {{ product.totalAmount }} € </span>
          ...
          <div class="products-payment-scroll-box">
            <div
              ...
              @click="handleScrollList('down', 'includedList')"
            >
              <span class="icon-arrow-thick-down"></span>
            </div>
            <div
              class="scroll-btn scroll-up"
              @click="handleScrollList('up', 'includedList')"
            >
        ...
        <div class="products-payment-box excluded-box">
          ...
          <div class="products-payment-scroll-box">
            <div
              class="scroll-btn scroll-down"
              @click="handleScrollList('down', 'excludedList')"
            >
              <span class="icon-arrow-thick-down"></span>
            </div>
            <div
              class="scroll-btn scroll-up"
              @click="handleScrollList('up', 'excludedList')"
            >
              <span class="icon-arrow-thick-up"></span>
            </div>
          </div>
    ...
    <div class="payment-info-smallscreen" v-if="isOnSmallScreen">
      <div class="payment-details-heading">
        ...
        <div class="total-payment">
          ...
          <div class="total-payment-num">
            {{ orderInfo.moneyToPay | centToEuroFormat }}
          </div>
        </div>
      </div>
      <div class="payment-info-bottom">
...
export default {
  ...
  computed: {
    ...
    orderInfo() {
      return {
        ...
        orderDiscountValue:
          this.orderSelected.orderDiscountType == "%"
            ? this.orderSelected.orderDiscountValue
            : Number(this.orderSelected.orderDiscountValue) * 100,
  ...
  methods: {
    ...
    handleClickOrderIncluded(product) {
      ...
      if (
        !product.isDisableSelect &&
        tmpOrderSelected.orderIncluded.length > 1
      ) {
        for (let i = 0; i < tmpOrderSelected.orderIncluded.length; i++) {
          if (
            product.code == tmpOrderSelected.orderIncluded[i].code &&
            !product.isDisableSelect
          ) {
    ...
    updateOrder(orderTmp) {
      ...
      for (let orderLine of tmpOrderIncluded) {
        ...
        totalPriceWithoutDiscount +=
          Number(orderLine.priceOrigin) * Number(orderLine.quantity);
      }
