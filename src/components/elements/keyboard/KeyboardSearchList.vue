<template>
  <div class="keyboard-wrapper" v-show="isActiveKeyboard">
    <div class="type-search-list-keyboard" v-show="isActiveSearchListKeyboard">
      <div class="search-list-keyboard"></div>
      <div class="close-keyboard" ref="searchList" @click="hideKeyboard()">Close</div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        isActiveSearchListKeyboard: state => state.helper.isActiveSearchListKeyboard,
      })
    },

    mounted() {
      this.keyboard = new Keyboard(".search-list-keyboard", {
    ...
    methods: {
      ...
      handleHeightCloseButton() {
        let searchListKb = document.querySelector('.type-search-list-keyboard .hg-button-space');
        let hSearchListKb = searchListKb.getBoundingClientRect().height;

        this.$refs.searchList.style.height = `${hSearchListKb}px`;
      }
    },
    watch: {
      input(input) {
