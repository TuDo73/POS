  ...
  <div class="popup-wrapper">
    <div class="popup-type type-move-dish" v-if="isActiveMoveDishPopup">
      ...
      <div class="popup-error">
        ...
        <h3 v-if="isTabOfTableTargetEmpty">
          Please select tab of table to move dish
        </h3>
      </div>
      <div class="popup-main">
        <div class="main-block">
          <div
            class="move-dish-box old-box"
            :class="{ 'disable-action': tableSelectedToMove == '' }"
          >
            ...
            <div class="list-body">
              <div class="list-body-wrapper">
                ...
                >
                  ...
                  <li class="row-item">{{ order.price }}€</li>
                  <li class="row-item">{{ order.totalAmount }}€</li>
                </ul>
              </div>
            </div>
            <div class="row-sum">
              <span>Sum:</span>
              <span class="sum-number"> {{ orderOrigin.totalPrice }}€ </span>
            </div>
          </div>
          ...
          <div class="move-dish-box new-box">
            <div class="box-heading">
              <div class="row-select list-tables">
                ...
                <select
                  v-model="tableSelectedToMove"
                  @change="changeTableTarget($event)"
                >
                  <option
                    v-for="(table, index) in tablesToMove"
                    :value="table.code"
                    :key="index"
                  >
                    {{ table.name }}
              ...
              <div class="row-select table-tabs" v-if="tableTabsTarget > 1">
                ...
                <div class="select-all-checkbox">
                  <select
                    v-model="tableTabTargetSelectedToMove"
                    @change="changeTableTabTarget($event)"
                  >
                    <option
                      v-for="(tab, index) in arrTableTabsTarget"
                      :value="tab.ref"
                      :key="index"
                    >
                      {{ index + 1 }}
            ...
            <div class="row-sum">
              <span>Sum:</span>
              <span class="sum-number"> {{ orderTarget.totalPrice }}€ </span>
            </div>
          </div>
        </div>
  ...
  methods: {
    ...
    changeTableTarget(event) {
      ...
      TableService.getAll().then(() => {
        ...
        OrderService.getOrdersByUser(this.userTableSelected).then((orders) => {
          ...
          this.arrTableTabsTarget = [];
          this.$store.commit(
            "order/setOrderTargetMoveDishEmptyOrderlines",
            this.orderSelected
          );
          for (let i = 0; i < orders.length; i++) {
            if (
              orders[i].userCode == this.userTableSelected &&
              orders[i].tableCode == this.tableSelectedToMove
            ) {
          ...
          if (this.tableTabsTarget == 1) {
            // nếu bàn target chỉ có 1 tab
            this.$store.commit(
              "order/setOrderTargetMoveDish",
              this.arrTableTabsTarget[0]
            );
          }
    ...
    changeTableTabTarget(event) {
      ...
      for (let i = 0; i < this.arrTableTabsTarget.length; i++) {
        if (
          this.arrTableTabsTarget[i].ref == this.tableTabTargetSelectedToMove
        ) {
          this.$store.commit(
            "order/setOrderTargetMoveDish",
            this.arrTableTabsTarget[i]
          );
        }
      }
    },
