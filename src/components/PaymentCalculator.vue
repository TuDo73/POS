...
export default {
  ...
  methods: {
    ...
    async goToPrint() {
      ...
      if (Number(orderSelectedTmp.moneyChange) < 0) {
        // không cho phép tiền trả nhỏ hơn tổng tiền hóa đơn
        this.$store.dispatch("popup/updatePopupState", true);
        this.$store.dispatch("payment/showHidePopupCheckValidPayment", true);
        return;
      }

      if (orderSelectedTmp.orderExcluded.length > 0) {
        // Xử lý nếu tách hóa đơn
        let newOrder = {};
